//! Embedded 8x8 pixel-art bitmap font covering ASCII 0x20..=0x7E.
//!
//! Extracted from the original `overlay.rs` so every UI layer shares one glyph
//! table. Index a glyph with `code - 0x20`; `glyph(ch)` does the bounds check.

/// Character cell width in pixels, before scaling.
pub const CHAR_W: usize = 8;
/// Character cell height in pixels, before scaling.
pub const CHAR_H: usize = 8;

/// Look up the 8-row bitmap for a character, or `None` if it is outside the
/// printable ASCII range the font covers.
